...
#       Recoded continuous function - parenting is more reliable, all cut sequences are detected and parented properly,
#           fades are now moved when a sequence is resized
...
#       Added QuickParenting option to auto-select child sequences, also child sequences' endpoints will be moved when
#           a parent is resized if they match up.
...
#       Rewrote the continuous function AGAIN, to now be a handler - this means movement/resizing of child clips is now
#           in real-time, and fades are continuously updated.  Also, switching scenes shouldn't have any adverse affect
#           on the script.
#       New variables added to sequence strips to help track changes: .last_frame_final_start, .last_frame_start,
#           .last_frame_final_duration, .last_select, and .new
...
#       Improved performance by only checking sequences in current editing scope (not checking sequences inside meta
#           strips if not in that meta strip)
#   0.91
...
#       Added option to correct the active strip when cutting (if you cut with mouse on the right, active strip remains
#           on left even tho right is selected)
...
#       Child strips now try to follow vertical movement of parent strips, but still get pushed out of the way by
#           other strips
...
#           New 'Delete' operator - can ripple delete, and can remove children, New ripple delete shortcut - Alt-Delete
#               and Alt-X
...
#       Replaced the sequencer Strip and Add menus so they can use the custom operators, also new option to simplify
#           strip menu by removing some items
...
#       Implemented Quick3Point - a basic 3point editing workflow, import from the file browser to the clip editor,
#           then to the sequencer.
#       Auto-Set Timeline Operator - move strips up to frame 1, set timeline start to frame 1, set timeline end to
#           last frame of last strip
#       The new cut operator now fixes effect sequence chains - it will duplicate single input effects (such as speed
#           control) to the newly cut sequence, and it will fix crossfades that should be applied to the right cut
#           sequence.  See https://developer.blender.org/T50877 for more info on the bug.
#       Now can save and recall zoom levels in the vse.  No way to do vertical (channel) zooms yet tho...
#       Right-click context menu option added, hold right click to activate it.  Options will differ depending on what
#           is clicked on - cursor, sequence, sequence handles, markers, empty area
